...
pub extern "C" fn rust_main(hart_id: usize) -> ! {
    ...
    let mutex = alloc::sync::Arc::new(async_mutex::AsyncMutex::new(0));
    let poll_twice = async_mutex::PollTwice::new();
    let task_4 = task::new_kernel(
        async_mutex::async_mutex_test0(alloc::sync::Arc::clone(&mutex), poll_twice),
        process.clone(),
        shared_payload.shared_scheduler,
        shared_payload.shared_set_task_state,
    );
    let task_5 = task::new_kernel(
        async_mutex::async_mutex_test1(mutex, alloc::sync::Arc::clone(&task_4)),
        process.clone(),
        shared_payload.shared_scheduler,
        shared_payload.shared_set_task_state,
    );
